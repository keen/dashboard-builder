...
const SettingsDashboard = props => {
  const builderOnChange = debounce(data => {
    console.log('builder on change', data);
    props.setTheme(data);
  }, 500);
...
const mapStateToProps = state => {
  const { dryRun, theme } = state.app.dashboardInfo.settings;
  const { is_public, id } = state.app.dashboardInfo;
  return {
    dryRun,
    theme,
    id,
