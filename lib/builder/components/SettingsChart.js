...
class SettingsChart extends Component {
  ...
  render() {
    const {
      settingsVisible,
      items: { savedQuery, sparkline, stacking, legend, prefix, suffix, showPoints, pointsSize },
      isLoading
    } = this.props;
    ...
    const showPointsValue = showPoints ? showPoints : { value: true, label: "Yes" };
          ...
          (type.includes('line') || type.includes('area') && !type.includes('step')) && (
            <React.Fragment>
              ...
              defaultValue={showPointsValue}
                value={showPoints}
                onChange={e =>
                  this.props.selectShowPoints(e, settingsVisible)
                }
