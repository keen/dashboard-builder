...
class SettingsChart extends Component {
  ...
  generateSettings = props => {
    if(props.dashboardData.savedQuery[0]){
      this.setState({
        loading: true,
      })
      client
        .get({
          url: client.url('queries', 'saved', props.dashboardData.savedQuery[0].value),
          api_key: client.masterKey()
        })
        .then(results => {
          const newType = ChartTypeUtils.getChartTypeOptions(results.query);
          const type = props.dashboardData.type ? props.dashboardData.type : newType[0];
          this.setState({
            results,
            type,
            loading: false
          ...
          if (this.state.type) {
            ...
            const chartListFiltered = chartList.filter(el => el.value === 'area' || el.value === 'line');;
            
            const availableChartTypes = props.dashboardData.savedQuery.length > 1 ? chartListFiltered : chartList
  ...
  componentWillReceiveProps(nextProps) {
    if (
      ...
      _.isEqual(nextProps.dashboardData.savedQuery, this.props.dashboardData.savedQuery) === false || nextProps.dashboardData.savedQuery.length === 0
    ) {
  ...
  render() {
    ...
    return (
      <div className="settings-chart">
        ...
        {savedQuery.length !== 0 && type !== "table" && results && (
          <React.Fragment>
            <h4>Sparkline:</h4>
            <Select
              value={sparkline}
