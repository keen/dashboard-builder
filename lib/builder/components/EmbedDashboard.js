...
const EmbedDashboards = props => {
  ...
  useEffect(() => {
    if (!props.accessKey) {
      getKeyFromAPI(
        ...
        props.title,
        keenAnalysis
      ).then(data => props.setAccessKey(data));
    }
  }, []);
  ...
  <html>
    <head>
      <meta charset="utf-8">
      <script crossorigin src="https://cdn.jsdelivr.net/npm/keen-dashboard-builder@${window.global.VERSION}/dist/viewer.min.js"></script>
    </head>
    <body>
      <div id="viewer"></div>
      <script>
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(EmbedDashboards)
);
