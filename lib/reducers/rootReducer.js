...
const appReducer = (state = defaultData, action) => {
  ...
  switch (action.type) {
    ...
    case actionTypes.LOAD_DASHBOARD_INFO:
      const { items } = action.dashboardInfo.data;
      const formattedItems = items.map(item => transformChart(item));
      loadFontsFromDashboard(action.dashboardInfo);

      const newDashboardInfo = {
        ...action.dashboardInfo,
        data: {
          ...action.dashboardInfo.data,
              ...
              ...dashboardInfo.data.items.map((el, i) =>
                action.index === i
                  ? {
                      ...el,
                      sparkline: false,
                      savedQuery: action.savedQueries,
                      error: false
                    }
                  : el
              )
