...
const editorReducer = (state = defaultData, action) => {
  ...
  switch (action.type) {
    ...
    case actionTypes.ADD_SAVE_QUERY_TO_LIST:
      return {
        ...state,
        dashboardInfo: {
          ...dashboardInfo,
    ...
    case actionTypes.DELETE_SAVE_QUERY_FROM_LIST:console.log(action.value)
      let delArray = [];
      if (dashboardInfo.settings.savedQueriesList) {
        const delIndex = dashboardInfo.settings.savedQueriesList.indexOf(
          action.value
        );
        delIndex >= 0
          ? (delArray = dashboardInfo.settings.savedQueriesList.splice(
              delIndex,
              1
            ))
          : (delArray = dashboardInfo.settings.savedQueriesList);console.log(action.value, delIndex)
      }
      return {
        ...state,
        dashboardInfo: {
