...
class EditorDashboard extends Component {
  ...
  render() {
    ...
    if(dashboardInfo.id !== '' && !!data){
      !data.items && !!rows && this.props.mapOldItems(editorParse(dashboardInfo));
      parsedItems = data.items
      parsedItems && parsedItems.forEach(el => {
      ...
      >
        {dashboardInfo.id !== '' && parsedItems &&
          parsedItems.map((el, index) => {
            ...
            let zIndex = "";
            if ((index === isMoving && isMoving !== false) || (index == isResizing && isResizing !== false)) {
              zIndex = 101;
            }
            let opacity = 1;
