...
class EditorDashboard extends PureComponent {
  ...
  createElement = (el) => {
    ...
    const { id, theme, charts_theme, version, layout, error, dryRun, settingsVisible } = this.props;
    const options = (charts_theme && charts_theme[i] && charts_theme[i].options) || (theme && theme.options) || {};
    ...
    return (
      <div key={i} data-grid={dataGrid} id={i} style={{opacity}} onDoubleClick={(e) => this.onElementClick(e, i)} onMouseDown={this.onMouseDown}>
        {type === 'image' ? (
          <Image src={src} />
        ) : type === 'paragraph' ? (
          <Paragraph text={text} />
        ...
        { version === 'editor' && <Buttons key={i} index={i} savedQuery={savedQuery} editedWidget={this.state.editedWidget} /> }
        {charts_theme &&
          charts_theme[i] &&
          charts_theme[i].style && (
            <style
              dangerouslySetInnerHTML={{
                __html: `${charts_theme[i].style}`
              }}
            ></style>
          )}
      </div>
  ...
  onDrop = elemParams => {
    ...
    const { id:dashboardId, counter = 0, draggedType, dropHandler } = this.props;
  ...
  render() {
    const { version, screenSize, id, items, layout, layouts, theme, ...options } = this.props;
    return (
      <div id={`dashboard-${id}`}>
        {theme && theme.style &&
          <style dangerouslySetInnerHTML={{__html: `${theme.style}`}}></style>
        }
...
const mapStateToProps = state => {
  const {
    dashboardInfo: {
      id,
      settings: {theme, charts_theme, layout, layouts, items, counter, savedQuery, fonts},
    },
    dashboardInfo,
    draggedType,
    settingsVisible,
