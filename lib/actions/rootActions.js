    ...
    .then(res =>
      res.json().then(data => {
        const parsingCheck = parseData => {
          if(!parseData.settings.items || (parseData.settings.items && !parseData.settings.items.length)){
            if(!parseData.data.items || (parseData.data.items && !parseData.data.version)) return classicDashboardDataParse(parseData, generateUniqueId);
...
export const selectSavedQuery = (queryNames, index) => (dispatch, getState) => {
...
export const deleteChart = index => (dispatch, getState) => {
