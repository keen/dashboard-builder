    ...
    .then(res =>
      res.json().then(data =>
        dispatch({
          ...
          dashboardList: data.data.items.length ? newGridDataParse(data) : classicDashboardDataParse(data)
        })
      )
    )
    .catch(err => console.error(err));
    ...
    .then(res =>
      res.json().then(data => {
        const parsingCheck = parseData => {
          if(!parseData.settings.items){
            if(!parseData.data.items) return classicDashboardDataParse(parseData, generateUniqueId());
