...
export const addDashboardItem = () => dispatch => {
  fetch(`https://staging.keen.io/projects/${client.projectId()}/dashboards/`, {
    method: "post",
...
export const deleteDashboardItem = id => dispatch => {
  fetch(
    `https://staging.keen.io/projects/${client.projectId()}/dashboards/${id}`,
    {
      method: "delete"
    }
  )
...
export const saveDashboard = dashboard => dispatch => {
  client
    .put({
      url: client.url("/dashboards", dashboard.id),
      api_key: client.masterKey(),
      params: dashboard
    })
    .then(res =>
